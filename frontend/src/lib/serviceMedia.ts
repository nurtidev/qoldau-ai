...
/**
 * Фокус кадра (CSS object-position) по ключу. Обложки карточек каталога и
 * плиток главной теперь имеют пропорцию 16/9 — ровно как исходные jpg, поэтому
 * object-fit:cover показывает кадр ЦЕЛИКОМ, без вертикального кропа (Y здесь
 * фактически не влияет). Значения ниже остаются осмысленными для квадратной
 * мини-обложки списочного вида (ServiceRow, 48×48): там кадр 16:9 обрезается по
 * бокам, работает координата X, а Y — safety на случай субпиксельного кропа.
 * Y-акценты сохранены по смысловому центру кадра (состав/комбайн/стадо).
 */
...
/** Дефолт для ключей без записи в FOCUS: при 16/9 кадр виден целиком, значение
 *  влияет лишь на квадратный thumbnail списка — держим чуть выше центра. */
