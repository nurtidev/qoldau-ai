...
// Первые два — контрольные кейсы ТЗ (вагоны в лизинг + агро-животноводство), default — вагоны.
const AI_PROMPT_PRESETS: { id: string; label: string; prompt: string }[] = [
  {
    id: 'brk-leasing',
    label: '⭐ ФРП — лизинг вагонов (контрольный кейс)',
    prompt: 'Приобретение авиатранспорта (Boeing/Airbus) и грузовых вагонов в лизинг от Фонда развития промышленности (бывш. БРК-Лизинг). Заявитель — юр. лицо РК. Нужны 6 шагов: (1) информация о компании — БИН из eGov, ОКЭД, дата регистрации, выручка за 3 года, численность; (2) предмет лизинга и казсодержание — тип (авиатранспорт/вагоны/спецтехника), наименование и модель, страна происхождения, доля казахстанского содержания (%), поставщик; (3) финансовая модель — стоимость, аванс ≥ 15%, сумма лизинга, ставка, срок, ежемесячный платёж (расчёт), DSCR ≥ 1.3 (расчёт по чистой прибыли / годовому платежу), прогнозная IRR ≥ 12%; (4) обеспечение и оценка — аккредитованный оценщик из реестра ФРП, страховая компания, описание залога; (5) KYC / ПОД-ФТ — бенефициары > 25%, источник средств, согласия на запросы в ПКБ, ГКБ, согласие на обработку персональных данных по Закону РК № 94-V, санкционная декларация; (6) документы — финансовая отчётность за 3 года (PDF), ТЭО проекта, протокол участников, СТ-KZ при наличии, аудиторское заключение (только для крупного бизнеса — условный шаг).',
  },
  {
    id: 'agro-livestock',
    label: '⭐ АКК — животноводство (контрольный кейс)',
    prompt: 'Финансирование развития животноводства от Аграрной кредитной корпорации: приобретение поголовья, строительство и модернизация ферм, субсидирование части затрат. Заявитель — КХ, ИП или юр. лицо РК. Нужны 6 шагов: (1) хозяйство и заявитель — ИИН/БИН из eGov, наименование/ФИО из eGov, форма хозяйства (КХ / ИП / ТОО / СПК), регион, стаж работы в животноводстве (лет); (2) направление животноводства — вид скота (КРС молочное / КРС мясное / МРС (овцы/козы) / Птица / Коневодство / Свиноводство), текущее поголовье (гол.), породность стада (товарное/племенное), порода или кросс птицы — условное поле, видно только при виде скота «Птица», порода/линия племенного стада — условное поле, видно только при породности «Племенное»; (3) параметры проекта — цель финансирования (приобретение поголовья / строительство фермы / модернизация / корма и оборудование), количество голов к приобретению, цена за голову, стоимость приобретаемого поголовья (расчёт = количество × цена), описание проекта строительства/модернизации фермы (textarea); (4) финансовая модель — запрашиваемая сумма финансирования, доля субсидирования затрат (%), сумма субсидии (расчёт = сумма × доля / 100), срок финансирования в месяцах, ежемесячный платёж по льготной ставке 6% годовых (расчёт); (5) обеспечение и соответствие — вид залога, оценочная стоимость залога, подтверждение отсутствия налоговой задолженности перед КГД (checkbox), подтверждение ветеринарного благополучия хозяйства — отсутствие карантинных ограничений (checkbox), дата последнего ветеринарного осмотра/вакцинации, дополнительные сведения о хозяйстве; (6) документы — этап 2, дозапрашивается после первичного рассмотрения: справка ИСЖ об идентификации сельскохозяйственных животных, ветеринарная справка о благополучии хозяйства, акт на право пользования землёй или договор аренды пастбищ, финансовая отчётность за последний год, бизнес-план проекта, справка КГД об отсутствии задолженности, отчёт независимого оценщика по залоговому имуществу.',
  },
  {
    ...
    <div className="page-fade" style={{
      ...
      <main style={{ flex: 1, overflowY: isBelowLaptop ? 'visible' : 'auto', minWidth: 0, width: '100%' }}>
        <div style={{
          ...
          <div style={{ flex: isBelowLaptop ? '1 1 100%' : '1 1 240px', minWidth: isBelowLaptop ? 0 : 220 }}>
            ...
            <div title={meta.title || 'Без названия'} style={{
              fontSize: 14, fontWeight: 600, wordBreak: 'break-word',
              display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden',
            }}>{meta.title || 'Без названия'}</div>
          </div>
