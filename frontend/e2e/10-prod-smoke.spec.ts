...
test('Калькулятор охвата открывается из конструктора и показывает счётчик', async ({ page }) => {
  ...
  // База синтетической аудитории ~3000 (миграция 015 добавила 7 демо-
  // пользователей → 3 007), поэтому матчим диапазон 3 0xx, как в 09-audience.
  await page.locator('text=/^3\\s?0\\d\\d$/').waitFor({ timeout: 15_000 })
})
